...
class ReligionPreferenceSheet extends StatefulWidget {
  final List<SimpleMasterData> list;
  final List<SimpleMasterData?> religionModel;
  const ReligionPreferenceSheet(
      {Key? key, required this.list, required this.religionModel})
      : super(key: key);

  @override
  _ReligionPreferenceSheetState createState() =>
      _ReligionPreferenceSheetState();
}

class _ReligionPreferenceSheetState extends State<ReligionPreferenceSheet> {
        ...
        child: Column(
          ...
          children: [
            ...
            MmmButtons.primaryButton("Done", () {
              Navigator.of(context).pop(this.widget.religionModel);
            })
          ],
        ),
        padding: kMargin16,
  ...
  bool isSelected(SimpleMasterData? value) {
    for (var item in widget.religionModel) {
  ...
  void setSelected(SimpleMasterData? value) {
    ...
    for (var item in widget.religionModel) {
      if (item!.title == value!.title) {
        isFound = true;
        index = widget.religionModel.indexOf(item);
      }
    }
    if (!isFound) {
      this.widget.religionModel.add(value);
    } else {
      this.widget.religionModel.removeAt(index);
    }
  }
}
