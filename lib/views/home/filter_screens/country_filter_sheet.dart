...
class CountryFilterSheet extends StatefulWidget {
  ...
  const CountryFilterSheet(
      {Key? key, required this.list, required this.countryModelList})
      : super(key: key);
...
class _CountryFilterSheetState extends State<CountryFilterSheet> {
        ...
        child: Column(
          ...
          children: [
            ...
            MmmButtons.primaryButton("Done", () {
              Navigator.of(context).pop(this.widget.countryModelList);
            })
          ],
        ),
        padding: kMargin16,
  ...
  bool isSelected(CountryModel? value) {
    for (var item in widget.countryModelList) {
  ...
  void setSelected(CountryModel? value) {
    bool isFound = false;
    int index = 0;
    for (var item in widget.countryModelList) {
      if (item!.name == value!.name) {
        isFound = true;
        index = widget.countryModelList.indexOf(item);
      }
    }
    if (!isFound) {
      this.widget.countryModelList.add(value);
    } else {
      this.widget.countryModelList.removeAt(index);
    }
  }
}
